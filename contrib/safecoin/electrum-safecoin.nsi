...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-Safecoin"
  !define PRODUCT_WEB_SITE "https://github.com/OleksandrBlack/electrum-safecoin"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-safecoin-${PRODUCT_VERSION}-setup-${BUILD_ARCH}.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrum-safecoin.ico"
...
Section
  ...
  File /r "dist\electrum-safecoin\*.*"
  File "icons\electrum-safecoin.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-safecoin-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-safecoin-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-safecoin-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-safecoin-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-safecoin-${PRODUCT_VERSION}.exe" 0


  ;Links safecoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\safecoin" "" "URL:safecoin Protocol"
  WriteRegStr HKCU "Software\Classes\safecoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\safecoin" "DefaultIcon" "$\"$INSTDIR\electrum-safecoin.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\safecoin\shell\open\command" "" "$\"$INSTDIR\electrum-safecoin-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-safecoin.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\safecoin"
