...
cp contrib/safecoin/osx.spec .
cp contrib/safecoin/pyi_runtimehook.py .
cp contrib/safecoin/pyi_tctl_runtimehook.py .

pyinstaller \
    -y \
    --name electrum-safecoin-$ELECTRUM_SAFE_VERSION.bin \
    osx.spec

sudo hdiutil create -fs HFS+ -volname "Electrum-Safecoin" \
    -srcfolder dist/Electrum-Safecoin.app \
    dist/electrum-safecoin-$ELECTRUM_SAFE_VERSION-macosx.dmg
